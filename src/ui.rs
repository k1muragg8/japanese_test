...
pub fn ui(f: &mut Frame, app: &App) {
    ...
    match app.state {
        ...
        AppState::Quiz => {
            let infinite_indicator = if app.due_count <= 0 { " [Infinite Mode]" } else { "" };
            let title = format!(" Quiz - Time: {}{} ", format_duration(app.session_start.elapsed()), infinite_indicator);

            // Strict separation: Quiz uses full screen split
